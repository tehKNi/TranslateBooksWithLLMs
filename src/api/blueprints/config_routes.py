...
def create_config_blueprint(server_session_id=None):
    ...
    def serve_interface():
        ...
        return (
            f"<h1>Error: Interface not found</h1><p>Looked in: {interface_path}</p>",
            404,
        )
    ...
    def get_available_models():
        ...
        if provider == "gemini":
            ...
            if request.method == "POST":
                api_endpoint = data.get(
                    "api_endpoint", "https://api.openai.com/v1/chat/completions"
                )
            else:
                api_endpoint = request.args.get(
                    "api_endpoint", "https://api.openai.com/v1/chat/completions"
                )
    ...
    def get_max_tokens():
        ...
        return jsonify({"max_tokens_per_chunk": MAX_TOKENS_PER_CHUNK})
    ...
    def _get_openrouter_models(provided_api_key=None):
        ...
        api_key = _resolve_api_key(
            provided_api_key, "OPENROUTER_API_KEY", OPENROUTER_API_KEY
        )

        # Use OPENROUTER_MODEL from .env, fallback to claude-sonnet-4
        default_model = (
            OPENROUTER_MODEL if OPENROUTER_MODEL else "anthropic/claude-sonnet-4"
        )
        ...
        try:
            ...
            models = asyncio.run(
                openrouter_provider.get_available_models(text_only=True)
            )
    ...
    def _get_deepseek_models(provided_api_key=None):
        ...
        api_key = _resolve_api_key(
            provided_api_key, "DEEPSEEK_API_KEY", DEEPSEEK_API_KEY
        )
    ...
    def _get_nim_models(provided_api_key=None):
        ...
        try:
            ...
            nim_provider = OpenAICompatibleProvider(
                api_key=api_key, model=default_model
            )
            models = asyncio.run(nim_provider.get_available_models())
                ...
                return jsonify(
                    {
                        ...
                        "status": "nim_error",
                        "count": 0,
                        "error": "Failed to retrieve NVIDIA NIM models",
                    }
                )
    ...
    def _get_ollama_models():
        ...
        ollama_base_from_ui = request.args.get(
            "api_endpoint", DEFAULT_OLLAMA_API_ENDPOINT
        )
                ...
                return jsonify(
                    {
                        ...
                        else (model_names[0] if model_names else DEFAULT_MODEL),
            ...
            return jsonify(
                {
                    ...
                    "is_uncontrollable": behavior == ThinkingBehavior.UNCONTROLLABLE
                    if behavior
                    else False,
                    "is_thinking_model": behavior
                    in [ThinkingBehavior.CONTROLLABLE, ThinkingBehavior.UNCONTROLLABLE]
                    if behavior
                    else False,
                }
            )
        ...
        try:
            ...
            for file_path in custom_instructions_dir.glob("*.txt"):
                ...
                try:
                    ...
                    txt_files.append(
                        {"filename": file_path.name, "display_name": file_path.stem}
                    )
                except ValueError:
            ...
            return jsonify(
                {"files": txt_files, "count": len(txt_files), "status": "ok"}
            )

        except Exception as e:
            logger.error(f"Error listing custom instructions: {e}")
            return jsonify(
                {"files": [], "count": 0, "status": "error", "error": str(e)}
            )
